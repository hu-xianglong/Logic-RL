...
MODEL_PATH=./checkpoints/7ppl/actor/global_step_450
export VLLM_ATTENTION_BACKEND=XFORMERS
python3 -m verl.trainer.main_ppo \
    algorithm.adv_estimator=grpo \
    data.train_files=data/kk/instruct/7ppl/train.parquet \
    data.val_files=data/kk/instruct/7ppl/test.parquet \
    ...
    trainer.experiment_name='Qwen-7B-7PPL-450-3EP' \
    trainer.n_gpus_per_node=4 \
    trainer.nnodes=1 \
    trainer.default_local_dir=./checkpoints/7ppl/second_run \
    trainer.default_hdfs_dir=null \
    trainer.save_freq=25 \
    trainer.test_freq=25 \
    trainer.total_epochs=3 $@ 2>&1 | tee grpo_7ppl_450.log
